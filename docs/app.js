const isHost = currentSession.host === currentUser.username
  ...
  function renderParticipants(){
    participantsList.innerHTML = currentSession.participants.map(p => `
      <li>
        ${p.displayName} 
        ${p.username === currentSession.host ? '<span class="host-badge">HOST</span>' : ''}
      </li>
    `).join('')
  }
  ...
  if(isHost){
    ...
    $('btn-change-host').addEventListener('click', ()=>{
      const others = currentSession.participants.filter(p => p.username !== currentUser.username)
      if(others.length === 0){
        alert('Não há outros participantes na mesa')
        return
      }
      
      const options = others.map((p, i) => `${i+1}. ${p.displayName}`).join('\n')
      const choice = prompt(`Transferir anfitrião para:\n\n${options}\n\nDigite o número:`)
...
REST = {
  ...
  menu: [
    { id: "m1", name: "Pizza Grande", price: 100.00, splitable: true, category: "Pratos" },
    { id: "m2", name: "Refil Refrigerante", price: 15.00, splitable: false, category: "Bebidas" },
    { id: "m3", name: "Batata Frita", price: 23.00, splitable: true, category: "Petiscos" },
    { id: "m4", name: "Salgado", price: 8.00, splitable: false, category: "Petiscos" },
    { id: "m5", name: "Hambúrguer Artesanal", price: 35.00, splitable: false, category: "Pratos" },
    { id: "m6", name: "Cerveja Long Neck", price: 12.00, splitable: false, category: "Bebidas" },
    { id: "m7", name: "Porção de Frango", price: 45.00, splitable: true, category: "Petiscos" },
    { id: "m8", name: "Suco Natural", price: 10.00, splitable: false, category: "Bebidas" },
    { id: "m9", name: "Sobremesa", price: 18.00, splitable: true, category: "Sobremesas" }
  ],
  settings: {
...
if(btnLogin){
  ...
  // Desabilitar botão até dados carregarem
  btnLogin.disabled = true
  btnLogin.textContent = 'Carregando...'
  
  loadData.then(() => {
    console.log('✅ Dados carregados, habilitando botão')
    btnLogin.disabled = false
    btnLogin.textContent = 'Entrar'
  }).catch(err => {
    console.error('❌ Erro ao carregar:', err)
    btnLogin.disabled = false
    btnLogin.textContent = 'Erro - Tentar Novamente'
    alert('Erro ao carregar dados da aplicação. Verifique se os arquivos users.json e restaurant.json estão na mesma pasta do index.html')
  })
  ...
  btnLogin.addEventListener('click', async ()=>{
    ...
    alert("Erro ao carregar dados. Recarregue a página e tente novamente.");
    return;
    }
...
if(userInfo){
  loadData.then(() => {
    const currentUser = JSON.parse(localStorage.getItem('mesalink_current_user')||'null')
    if(!currentUser){ 
...
function initMesa(currentUser){
  ...
  for(let code in sessions){
    if(sessions[code].participants.some(p => p.username === currentUser.username)){
...
function continueInitMesa(currentSession, currentUser){
        ...
        ${items.map(item => `
          <div class="menu-item" style="margin-bottom:8px">
            <div>
              ...
              ${item.splitable ? '<br><span style="color:var(--accent);font-size:11px">✓ Divisível</span>' : ''}
            </div>
  ...
  window.removeFromCart = (orderId) => {
    const idx = currentSession.orders[currentUser.username].findIndex(item => item.orderId === orderId)
    if(idx !== -1){
  ...
  window.toggleSplit = (orderId) => {
    const item = currentSession.orders[currentUser.username].find(i => i.orderId === orderId)
  ...
  function renderCart(){
    ...
    cartList.innerHTML = myOrders.map((item) => {
      const splitCount = item.splitWith.length + 1 // +1 for current user
      const splitPrice = item.splitable && item.splitWith.length > 0 ? item.price / splitCount : item.price
          ...
          <div style="display:flex;justify-content:space-between;width:100%;margin-bottom:4px">
            <div>
              ...
              ${item.splitWith.length > 0 ? 
                `<br><span style="color:var(--accent);font-size:11px">Dividido por ${splitCount} = R$ ${splitPrice.toFixed(2)}</span>` 
                : ''}
            </div>
            <button onclick="removeFromCart(${item.orderId})" style="padding:4px 8px">✕</button>
          </div>
          ${item.splitable ? `
            <button onclick="toggleSplit(${item.orderId})" style="width:100%;padding:6px;font-size:12px;background:${item.splitWith.length > 0 ? '#10b981' : 'rgba(255,255,255,0.1)'};margin-top:4px">
  ...
  btnCalc.addEventListener('click', ()=>{
    ...
    for(let username in currentSession.orders){
      ...
      userOrders.forEach(item => {
        ...
        if(item.splitable && item.splitWith && item.splitWith.length > 0){
          ...
          // Add to orderer
          breakdown[username].items.push(`${item.name} (${splitCount}x) - R$ ${splitPrice.toFixed(2)}`)
          breakdown[username].subtotal += splitPrice
          
          // Add to split participants
          item.splitWith.forEach(splitUser => {
            if(breakdown[splitUser]){
              breakdown[splitUser].items.push(`${item.name} (${splitCount}x) - R$ ${splitPrice.toFixed(2)}`)
              breakdown[splitUser].subtotal += splitPrice
            }
          })
        } else {
          // Item not split
          breakdown[username].items.push(`${item.name} - R$ ${fullPrice.toFixed(2)}`)
    ...
    let detailedBreakdown = '<div style="max-height:200px;overflow-y:auto;margin-bottom:12px;padding:8px;background:rgba(0,0,0,0.2);border-radius:6px">'
    ...
    for(let username in breakdown){
      ...
      detailedBreakdown += `
        <div style="margin-bottom:12px;padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,0.1)">
          <strong style="color:${username === currentUser.username ? 'var(--accent)' : 'inherit'}">${user.displayName}:</strong><br>
          ${user.items.map(item => `<span class="muted small">• ${item}</span><br>`).join('')}
    ...
    totals.innerHTML = detailedBreakdown + `
      ...
      <div style="margin-top:12px;padding-top:12px;border-top:2px solid var(--accent)">
        <strong style="font-size:20px;color:var(--accent)">Você paga: R$ ${myTotalWithTip.toFixed(2)}</strong>
      </div>
      ...
      if(window.paymentModal){
        window.paymentModal.innerHTML = `
          <div style="background:var(--bg);padding:32px;border-radius:16px;max-width:400px;width:100%;border:2px solid #10b981;text-align:center">
            ...
            <p class="muted">Pagamento de <strong>R$ ${amount.toFixed(2)}</strong> via ${methodNames[method]}</p>
            <p class="muted" style="margin-top:16px">Comprovante: #${Math.random().toString(36).substr(2, 9).toUpperCase()}</p>
...
function renderActiveSessions(){
  ...
  if(sessionKeys.length === 0){
    activeSessionsList.innerHTML = '<li class="muted">Nenhuma mesa ativa no momento</li>'
    setTimeout(renderActiveSessions, 5000)
    return
  }
  
  const html = sessionKeys.map(code => {
    ...
    return `
      <li style="margin-bottom:12px;padding:12px;background:rgba(255,255,255,0.02);border-radius:8px">
        ...
        <span class="muted">Participantes: ${s.participants.map(p=>p.displayName).join(', ')}</span><br>
        <strong style="color:var(--accent)">Total: R$ ${totalTable.toFixed(2)}</strong>
      </li>
    `
  }).join('')
